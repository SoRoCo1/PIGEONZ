...
export default function ProjetosPage() {
  ...
  const [newTipo, setNewTipo] = useState<"cliente" | "parceiro">("cliente");
  const [newEmail, setNewEmail] = useState("");
  const [newPlano, setNewPlano] = useState(0); // index into PLANS
  const [creating, setCreating] = useState(false);

  const processPropostas = (allData: Record<string, Record<string, unknown>>) => {
    const isParceriaOrFree = (p: PropostaEntry) =>
      p.selectedPlan === 2 || p.selectedPlan === 3 || p.userEmail === "bluezonesalesmkt@gmail.com";
    const list = (Object.values(allData) as unknown as PropostaEntry[])
      .filter(p => p.userRole === "cliente" && (p.pagamentoConfirmado || isParceriaOrFree(p)))
      .sort((a, b) => {
  ...
  const criarProjeto = async () => {
    ...
    await setPropostaDB(newEmail, {
      ...
      cupomRef: "",
      cupomAplicado: false,
      pagamentoConfirmado: newPlano === 2, // Parceria = auto confirmed
      comments: [],
  ...
  const gerenciarProjeto = (email: string) => {
    // Save which user the admin wants to manage, then navigate
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <p style={{ fontFamily: "'VT323', monospace", fontSize: 20, color: "#ffffff", textShadow: "-2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000, 2px 2px 0 #000, 0 -2px 0 #000, 0 2px 0 #000, -2px 0 0 #000, 2px 0 0 #000", marginBottom: 24 }}>
          Todos os projetos de clientes e parceiros.
        </p>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              ...
              border: `2px solid ${K}`, cursor: "pointer",
        ...
        {showCreate && (
          <div className="pixel-box" style={{ background: "rgba(10,8,20,0.95)", padding: 24, marginBottom: 20 }}>
            <h3 style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 9, color: K, letterSpacing: 1, marginBottom: 16 }}>
              CRIAR NOVO PROJETO
            </h3>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {(["cliente", "parceiro"] as const).map(t => (
                  <button key={t} onClick={() => { setNewTipo(t); setNewEmail(""); }}
                    style={{
                      fontFamily: "'VT323', monospace", fontSize: 16,
                      padding: "8px 20px", cursor: "pointer",
                      color: newTipo === t ? "#1c1030" : (t === "cliente" ? K : T),
                      background: newTipo === t ? (t === "cliente" ? K : T) : "transparent",
                      border: `2px solid ${t === "cliente" ? K : T}`, transition: "all 0.2s",
                    }}
                  >
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 7, color: "#8878a8", display: "block", marginBottom: 6, letterSpacing: 1 }}>
                {newTipo === "cliente" ? "EMAIL DO CLIENTE" : "EMAIL DO PARCEIRO"}
              </label>
              <select
                value={newEmail}
                    ...
                    <div style={{ flex: 1, minWidth: 200 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                        ...
                        <span style={{
                          ...
                          color: p.userRole === "parceiro" ? T : "#8878a8",
                          border: `1px solid ${p.userRole === "parceiro" ? T : "#8878a8"}`,
                          padding: "1px 6px",
                        }}>
                          {p.userRole === "parceiro" ? "Parceiro" : "Cliente"}
                        </span>
                      </div>
                      ...
                      {p.cupomAplicado && p.cupomRef && (
                        <span style={{ fontFamily: "'VT323', monospace", fontSize: 14, color: G }}>
                          Indicado por: {p.cupomRef}
                        </span>
                      )}
                      {p.siteLink && (
